 ...
 */
async function initiatePayment({ amount, customerMsisdn, description, callbackUrl, reference }) {
  ...
  const body = {
    ...
    requestingOrganisationTransactionReference: reference || `REF-${Date.now()}`,
  ...
  const response = await axios.post(
    `${baseUrl}/mvola/mm/transactions/type/merchantpay/1.0/`,
    body,
    { headers }
  );
...
async function getTransactionStatus(serverCorrelationId) {
  ...
  const response = await axios.get(
    `${baseUrl}/mvola/mm/transactions/type/merchantpay/1.0/status/${serverCorrelationId}`,
    { headers }
  );
...
async function getTransactionDetails(transactionId) {
  ...
  const response = await axios.get(
    `${baseUrl}/mvola/mm/transactions/type/merchantpay/1.0/${transactionId}`,
    { headers }
  );
 ...
 */
async function pollUntilComplete(serverCorrelationId, { interval = 3000, timeout = 120_000 } = {}) {
  ...
  while (Date.now() - start < timeout) {
    ...
    console.log(`⏳ Statut: ${status.status} – nouvel essai dans ${interval / 1000}s…`);
    await new Promise((r) => setTimeout(r, interval));
  }

  throw new Error("⏰ Timeout : la transaction n'a pas abouti dans le délai imparti.");
}
...
function handleMvolaError(error) {
  if (error.response) {
    ...
    const msg = errorMessages[fault?.code] || fault?.message || "Erreur inconnue";
